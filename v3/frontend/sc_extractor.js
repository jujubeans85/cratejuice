<script>
/* ===============================
   CJ — SoundCloud Extractor (One-Button)
   =============================== */
...
const CJ_SC = (() => {
  ...
  // Core extractor that runs in-page (must be on a soundcloud.com page)
  async function runExtractor({ enrich = true, limit = 300 } = {}) {
    // Helper: filter + normalize SoundCloud links
    const scFilter = (u) => {
      try {
  ...
  // Injects a simple panel into CJ (for your /v3 frontend)
  function mountPanel(containerId = 'cj-sc-panel') {
    ...
    wrap.innerHTML = `
      <div style="display:flex;gap:8px;align-items:center;justify-content:space-between;margin-bottom:8px">
        ...
        <label style="display:flex;align-items:center;gap:6px;opacity:.8">
          <input type="checkbox" id="cj_enrich" ${STATE.enrich ? 'checked':''}/> Enrich titles
        </label>
      </div>
    ...
    const { onSC } = uiReady();
    document.getElementById('cj_enrich').onchange = (e)=>{ STATE.enrich = !!e.target.checked; updateBookmarkletLink(); };
    document.getElementById('cj_run').onclick = async ()=>{
      if (!onSC) {
        alert('Open this on a SoundCloud page (soundcloud.com) and try again. Use the bookmarklet otherwise.');
        return;
      }
      runExtractor({ enrich: STATE.enrich, limit: STATE.limit });
    };
    document.getElementById('cj_copy').onclick = async ()=>{
      const bm = makeBookmarklet(STATE.enrich, STATE.limit);
      try { await navigator.clipboard.writeText(bm); alert('Bookmarklet copied. Create a new bookmark and paste into the URL field.'); }
