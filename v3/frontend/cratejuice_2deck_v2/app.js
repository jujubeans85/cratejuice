...
function wireDeck(btnPlayId, btnStopId, audioEl, pitchId, statusId, track, setLocalFlag){
  ...
  pitch.addEventListener('input', ()=>{
    audioEl.playbackRate = parseFloat(pitch.value || 1);
  });

  btnPlay.addEventListener('click', ()=>{
    if(setLocalFlag()){ // local available
      audioEl.play().catch(()=>{});
    }else{
      // clean direct open in new tab
  ...
  // probe local and set source
  probe(track.local).then(ok=>{
    if(ok){
      audioEl.src = track.local;
      status.textContent = 'Source: Local file';
      setLocalFlag(true);
    }else{
      status.textContent = 'Source: Spotify';
      setLocalFlag(false);
    }
  });
}
...
const xfader = () => {
  ...
  const apply = ()=>{
    ...
    const a = (v < 0) ? 1 : 1 - v;        // more left when negative
    const b = (v > 0) ? 1 : 1 + v;        // more right when positive
    // Normalize to 0..1
    const Avol = Math.max(0, Math.min(1, a));
    const Bvol = Math.max(0, Math.min(1, b));
    A.volume = Avol;
    B.volume = Bvol;
  };
...
document.addEventListener('DOMContentLoaded', ()=>{
  wireDeck('playMimi','stopMimi',A,'pitchA','statusA',TRACKS.mimi,(val)=>{
    if(typeof val==='boolean') A_hasLocal = val;
    return A_hasLocal;
  });
  wireDeck('playCbo','stopCbo',B,'pitchB','statusB',TRACKS.cbo,(val)=>{
    if(typeof val==='boolean') B_hasLocal = val;
    return B_hasLocal;
  });
  xfader();
});
