...
def main():
    ...
    if not tracks:
        print("No tracks found. Drop MP3s into offgrid-crates/ and run ./crate_run.sh first.")
        sys.exit(1)
    ...
    while True:
        ...
        if which("ffplay"): play_ffplay(path, args.rate)
        elif which("mpg123"): play_mpg123(path, args.rate)
        else:
